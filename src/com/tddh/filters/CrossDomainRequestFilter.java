...
/**
 * cross domain request filter Servlet Filter implementation class
 * CrossDomainRequestFilter
 */
public class CrossDomainRequestFilter implements Filter {
	 ...
	 * Default constructor.
	 */
	public CrossDomainRequestFilter() {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		String jsonCallback = ((HttpServletRequest) request).getParameter("jsonpCallback");
		if (jsonCallback != null) {
			...
			// OPTIONS方法不要拦截，不然跨域设置不成功
			// // do something
			// System.out.println("//OPTIONS方法不要拦截，不然跨域设置不成功");
			// }
			chain.doFilter(request, response);
		}
	}
